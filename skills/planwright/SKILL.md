...
re-surveys and an undrained frontier hardens instead. After the post-growth harden re-converges, a non-`safe` drive runs one **qb intent-replan** (`/qb-plan auto`, when qb is installed) as the closing escalation rung — merging its pending items into the plan (deduped against completed/rejected, re-validated by planwright's own validator) and executing them; if qb discloses a non-independent in-session audit (`QB_PLAN_AUTO_WARN`), codmaster downgrades trust — it independently re-validates and verifies every merged item regardless of qb's verdict, and treats **any** convergence the WARN-tagged qb rung closed (dry, or merged-then-re-converged) as unverified (a non-independent audit can rubber-stamp a real gap — a false "0 items" or a silently incomplete result): it still stops, but the REPORT keeps the stop reason and marks that final point non-independent and recommends an independent re-run rather than asserting a clean done; a `loop` drive also keeps a sticky drive-level marker that never re-arms on relap, so a WARN-tainted progress-making lap is disclosed at drive end and cannot be laundered into a clean done by a later independent lap. Its `advise` word prints the recommendation
...
When `hybrid-ai` is **on**, this dossier survey is **delegated** to the external-agent CLI backend —
the **same** entirely-optional backend Stage 1.6's `parallel external` uses, resolved and preflighted
identically (`run-agent.sh --check`) — via `run-agent.sh --agent all --read-only --target <T>`, where
`<T>` is the **smallest directory enclosing the run's Focus** (the `path` scope when it is a directory,
else the common-ancestor directory of the Focus paths; the repo root only for a whole-repo run). This
