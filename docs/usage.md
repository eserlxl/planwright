...
fixed catalog `power-user → integration → onboarding → reliability → automation` (then wrapping). Cycle
...
- **Commands** — the recommended next sweep for the current state (codvisor / codinventor / codcycle),
  ...
  **run in progress**, carries the Console beacon's running-command line, and frames the
  pick beneath with a **next dispatch once this run settles** label; a stale beacon (a dead
  run's leftover) keeps the next-dispatch framing.
...
- **Fleet** — a portfolio glance across every project the single dashboard server tracks
...
- **`--select EXPR`** — print the repo-relative paths of nodes matching a predicate (one per line, sorted) for scripting without `jq`. EXPR is one of `is_articulation`, `covered_by_test`, `is_test`, a `no-` negation of those, `code` (`branch_count > 0`), `oversized` (`loc > 300` — the Stage 2a oversized-module threshold; compose `oversized,no-covered_by_test` to triage large untested files), `swallows` (`swallow_count > 0` — the silent-failure analogue of `code`; compose `swallows,no-covered_by_test` to triage untested error-swallowing sites), `branch>N` / `swallow>N` / `loc>N` (numeric thresholds on the per-node `branch_count` / `swallow_count` / `loc` counters — the graded form of `code`/`swallows`/`oversized` at any cutoff, e.g. `branch>50` for the branch-densest, hardest-to-test code or `loc>500` for the largest files), `never-audited` (no reachable audit stamp — the same bin `frontier.never_audited` counts), `stale-audited` (stamped, but stamped before HEAD — the raw per-node form of the frontier's `stale` bin; compose `stale-audited,code,no-is_test` to reproduce that count), `dirty` (the incremental-audit dirty set — changed nodes plus their 1-hop blast radius, the working set Stages 3–7 gate on; compose `dirty,no-covered_by_test` to find changed code lacking a test), or `lang=NAME` — or a **comma-ANDed conjunction** of those (a node must match every member), so a multi-signal slice needs no `jq` post-processing either. **The two audit-stamp predicates and `dirty` are meaningful only with `--prior .planwright/graph.json`** — the stamps and the dirty diff live in the graph memory, and a fresh build has no baseline, so without `--prior` the slice is silently degenerate (`never-audited` and `dirty` match every node — a fresh build treats the whole tree as never-audited and first-run-dirty — and `stale-audited` matches none). It takes precedence over `--dot`:
  ...
  python3 <scripts>/build-graph.py --select oversized,no-covered_by_test # large (>300 loc) untested files
  python3 <scripts>/build-graph.py --select swallows,no-covered_by_test # untested silent-failure sites
  python3 <scripts>/build-graph.py --select 'branch>50'             # the branch-densest (untestable) code (quote the >)
  python3 <scripts>/build-graph.py --select 'loc>500'               # the largest files, at any cutoff (graded oversized)
