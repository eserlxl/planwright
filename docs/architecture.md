...
- **Import edges** extracted with `rg` per language family (bash/python/js/c/rust/go/markdown), best-effort. Comments and Python docstrings are stripped first (`strip_comments`) so a commented-out or string-embedded import does not create a false edge. (Go intra-module imports resolve via each `.go` file's nearest enclosing `go.mod`, root or nested sub-module — the module prefix is stripped and the remainder mapped, relative to that module's directory, to the package directory's `.go` files; stdlib, external, and cross-module imports drop.)
