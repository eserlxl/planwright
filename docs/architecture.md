...
- **Import edges** extracted with `rg` per language family (bash/python/js/c/rust/go/markdown), best-effort. Comments and Python docstrings are stripped first (`strip_comments`) so a commented-out or string-embedded import does not create a false edge. (Go intra-module imports resolve via the root `go.mod` module path — the module prefix is stripped and the remainder mapped to the package directory's `.go` files; stdlib, external, and nested-sub-module imports drop.)
