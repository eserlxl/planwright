...
- **Import edges** extracted with `rg` per language family (bash/python/js/c/rust/go/markdown), best-effort. Comments and Python docstrings are stripped first (`strip_comments`) so a commented-out or string-embedded import does not create a false edge. (Go intra-module imports resolve via each `.go` file's nearest enclosing `go.mod`, root or nested sub-module — the module prefix is stripped and the remainder mapped, relative to that module's directory, to the package directory's `.go` files; stdlib, external, and cross-module imports drop. C/C++ resolves both quoted `#include "x"` and angle `#include <project/foo.h>` includes — an angle include matches the unique repo file ending in that sub-path (an -I include root), strictly, so extensionless or unmatched system headers like `<vector>`/`<sys/types.h>` drop instead of forging a false edge.)
...
### Stage 11: Write
Appends the surviving items into `.planwright/plan.md`, then runs `scripts/lint-plan.py` on the written plan and fixes any reported violation before finishing. (If `--dry-run` is active, it lints the would-be items the same way and only prints the plan to chat.) The execute and cycle paths re-run the same linter as a precondition, so a structurally invalid plan never reaches the mutating loop. The linter also offers `--fix`, which auto-corrects the two mechanical, filesystem-verifiable violations in place (a `CMakeLists` surface respelled `CMakeLists.txt`; an already-existing `New Surface` moved into `Surfaces`) and leaves every judgement-dependent violation for the agent.
