...
`lint-plan.py` is unaffected at first (the Surfaces-in-Focus check stays Claude's Stage 10 judgement);
a later `lint-plan.py --scope <focus-list>` could mechanize it once the resolution rules settle.
