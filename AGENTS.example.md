...
- Opt-in `hybrid-ai` (a base flag the `cod*` sweeps forward) delegates the Stages 3-7 dossier survey to the **optional external-agents** CLIs (agy/codex) off the host agent — never-Evidence and off==skipped; planwright never requires them (opt-in, public-repo egress, never private IP). See `docs/hybrid-ai-design.md`
