...
desc_for() {
  case "$1" in
    ...
    codmaster)  echo "Personal alias for /planwright:codmaster — the front door: senses the planning state and runs the required commands consecutively to the final point (advise = tell only; safe = no invention) without the plugin prefix." ;;
  esac
}
arghint_for() {
  case "$1" in
    ...
    codmaster)  echo "advise | safe | (empty = sense → dispatch → re-sense, consecutively until the final point)" ;;
  esac
}
