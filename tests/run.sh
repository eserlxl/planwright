...
CASES="statics-scaffold skill-contract skill-guards skill-graph-contract build-graph unit-engine lint-plan lint-final lifecycle doctor pr status cwd-relative state registry dashboard derive check-links golden-plan integration-scale commands install-aliases make-plugin bump-version security"
