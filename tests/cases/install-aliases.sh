...
# (/codcycle, /codvisor, /codinventor, /codshard) into a Claude commands dir.
...
# that it removes only the four aliases and leaves unrelated files alone.
#
...
# --- Test IA1: --dir installs the four delegator aliases ------------------
if bash "$IA" --dir "$IADIR" >"$TMP/ia.out" 2>"$TMP/ia.err"; then
  ia_missing=""
  for n in codcycle codvisor codinventor codshard; do
    [ -f "$IADIR/$n.md" ] || ia_missing="$ia_missing $n"
  done
  if [ -z "$ia_missing" ]; then
    ok "install-aliases --dir writes the four delegator alias files"
  else
...
# --- Test IA3: --uninstall removes only the four aliases, leaving other files -
printf 'keep me\n' > "$IADIR/other.md"
if bash "$IA" --dir "$IADIR" --uninstall >"$TMP/ia.un" 2>&1; then
  ...
  if [ "$ia_gone" -eq 1 ] && [ -f "$IADIR/other.md" ]; then
    ok "install-aliases --uninstall removes the four aliases and leaves unrelated files"
  else
