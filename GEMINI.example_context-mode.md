...
- `codshard [args]`: The sharded maturity sweep. Follow the orchestration recipe in `/absolute/path/to/planwright/commands/codshard.md`: one scoped `cycle 3 depth 10` round per shard sequentially (staleness order), then one closing whole-repo round; each round is an ordinary planwright run. An opt-in `parallel` prefetches read-only recon leads per shard via the host's native subagent backend, routing-only and never Evidence (rounds stay sequential); an explicit `parallel external` opts into the **optional external-agents** CLIs (agy/codex) for host-neutral recon off Claude Code — planwright never requires them, and they ship the tree to external providers (opt-in, never private IP).
...
- `codpr [handoff | <N>]`: PR ingest. The planwright `pr` subcommand — every form prefixes `pr` (empty → `planwright pr`, `handoff` → `planwright pr handoff`, `123` → `planwright pr 123`). Ingests the current branch's open PR (unresolved review threads + failing CI) into grounded plan items, re-grounding every anchor against the live tree; `handoff` prints the local push-back recipe. Read-only toward GitHub — planwright never pushes, comments, resolves, or merges; the operator does that by hand (merging the PR is the close).
- Opt-in `hybrid-ai` (a base flag the `cod*` shortcuts forward) delegates the Stages 3-7 dossier survey to the **optional external-agents** CLIs (agy/codex) off the host agent — never-Evidence and off==skipped; planwright never requires them (opt-in, public-repo egress, never private IP). See `/absolute/path/to/planwright/docs/hybrid-ai-design.md`.
